...
def main(assigned_args: list = None):
    ...
    parser.add_argument("-p", "--privilege", dest="privilege", metavar="PRIVILEGE", type=bool, default=False, help="Only for UNIX system : Define the type of socket based on privilege level : True is SOCK_RAW False is SOCK_DGRAM")
    ...
    for addr in args.dest_addr:
        ping3.verbose_ping(addr, count=args.count, ttl=args.ttl, timeout=args.timeout, size=args.size, interval=args.interval, privilege=args.privilege)
